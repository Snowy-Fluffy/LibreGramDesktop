					...
					if (PathCanonicalize(exp, locationStr)) {
						...
						if (GetFullPathName(L".", bufSize, exp, 0) < bufSize) {
							...
							if (installpath == mypath + L"\\" || true) { // always update reg info, if we found it
								...
								const WCHAR *appURL = L"https://github.com/libregram/libregram";
...
static const WCHAR *_programName = L"LibreGram"; // folder in APPDATA, if current path is unavailable for writing
